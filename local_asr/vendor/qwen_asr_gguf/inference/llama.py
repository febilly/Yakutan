...
def _llama_primary_dll_basename() -> str:
...
def _resolve_llama_lib_dir() -> str:
    """优先 YAKUTAN_QWEN_LLAMA_BIN（如 local_asr_models/qwen_llama_vulkan_bin），否则为包内 inference/bin。"""
    default = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bin")
    env = os.environ.get("YAKUTAN_QWEN_LLAMA_BIN", "").strip()

    def usable(d: str) -> bool:
        if not d or not os.path.isdir(d):
            return False
        return os.path.isfile(os.path.join(d, _llama_primary_dll_basename()))
...
def load_model(model_path: str):
    ...
    """
    lib_dir = Path(_resolve_llama_lib_dir())
...
class LlamaContext:
    ...
    def decode_token(self, token_id, pos: int | None = None):
        """
        原子操作：设置单 Token Batch 并执行解码
        """
        if pos is None:
            return self.decode(get_one_batch(token_id))
        return self.decode(get_one_batch_at_pos(token_id, pos))
...
def get_one_batch_at_pos(token_id: int, pos: int, seq_id: int = 0):
    """
    为单 Token 显式指定位置，避免依赖 llama_batch_get_one 的自动位置推断。
    """
    batch = LlamaBatch(1, 0, 1)
