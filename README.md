...
- 断句：
    ...
    - 快速开关麦克风清空聊天框：在短时间内（默认0.8秒）连续两次闭麦，会清空 VRChat 聊天框中已发送的内容，并撤回在途的识别/翻译结果
...
- WebUI 提供简易和高级两种界面模式
    - 简易模式：只显示最常用的核心控件（识别、翻译、语言、麦克风），适合快速上手
    - 高级模式：左右两栏卡片，所有功能可见，适合精细调参
    - 模式切换会记住上次选择，简易模式下被隐藏的高级控件仍可通过 main.js 的 id 引用正常工作
