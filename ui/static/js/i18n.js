...
const translations = {
    'zh-CN': {
        ...
        'hint.llmParallelFastestMode': '启用后会并行发送两个相同请求并采用先返回的结果；开启流式翻译时仅对最后终译生效。请注意这会增加 token 用量。',
    ...
    'en': {
        ...
        'hint.llmParallelFastestMode': 'When enabled, two identical requests are sent in parallel and the first returned result is used. In streaming mode, this applies only to the final translation. Note: this increases token usage.',
    ...
    'ja': {
        ...
        'hint.llmParallelFastestMode': '有効にすると同じリクエストを 2 本並列送信し、先に返った結果を採用します。ストリーミング翻訳時は最終終訳にのみ適用されます。token 使用量が増える点に注意してください。',
    ...
    'ko': {
        ...
        'hint.llmParallelFastestMode': '활성화하면 동일한 요청 2개를 병렬로 보내고 먼저 돌아온 결과를 사용합니다. 스트리밍 번역에서는 최종 번역에만 적용됩니다. 토큰 사용량이 증가할 수 있습니다.',
