...
const translations = {
    'zh-CN': {
        ...
        'btn.stopping': '停止中...',
        'btn.openPanel': '小面板',
        'btn.copyVrcxBridgeScript': '复制 VRCX 上下文脚本',
        'btn.copying': '复制中...',
        'btn.clearLanguageInput': '清除输入',
        'label.floatingPanelMode': '悬浮窗模式',
        'status.vrcxBridgeIdle': '尚未接收 VRCX 上下文',
        'status.vrcxBridgeWaiting': '脚本已复制，等待 VRCX 推送上下文',
        'status.vrcxBridgeConnected': '已接收 VRCX 上下文（{seconds} 秒前）',
        'status.vrcxBridgeStale': 'VRCX 上下文已过期，请重新运行脚本',
        ...
        'label.enableArabicReshaper': '阿拉伯文/希伯来文重排',
        'hint.enableArabicReshaper': '让阿拉伯文和希伯来文在 VRChat 中按正确字形和方向显示，并限制 RTL 手动换行',
        ...
        'btn.llmTemplateDeepSeekV4Flash': 'DeepSeek v4 Flash（推荐）',
        ...
        'label.showVrcxBridgeButton': '显示 VRCX 上下文桥接按钮',
        'hint.showVrcxBridgeButton': '开启后可复制脚本到 VRCX 控制台，让当前世界和玩家名辅助翻译',
        'subsection.vad': 'VAD（语音活动检测）',
        'label.vadEnabled': '启用 VAD',
        'hint.vadEnabled': '总开关，同时控制在线 API 与本地识别。在线 API：静音时暂停发送音频以省流；本地识别：用于自动分段。',
        'label.vadMode': 'VAD 模式（本地识别）',
        'hint.vadMode': '本地识别使用的 VAD 算法；在线 API 门控固定使用 Silero。',
        'label.vadThreshold': 'VAD 阈值（0.0-1.0）',
        'hint.vadThreshold': '值越大越严格，越不容易把噪声判为语音',
        'hint.localVadMovedToAdvanced': 'VAD（语音活动检测）参数已移至「高级设置 → VAD」，对在线 API 与本地识别统一生效。',
        ...
        'msg.vrcxBridgeScriptCopied': 'VRCX 上下文脚本已复制。请粘贴到 VRCX 主界面的控制台运行。',
        'msg.vrcxBridgeScriptCopyFailed': '复制 VRCX 上下文脚本失败',
    ...
    'en': {
        ...
        'btn.copyVrcxBridgeScript': 'Copy VRCX Context Script',
        'btn.copying': 'Copying...',
        'btn.clearLanguageInput': 'Clear input',
        'label.floatingPanelMode': 'Floating Panel Mode',
        'status.vrcxBridgeIdle': 'No VRCX context received yet',
        'status.vrcxBridgeWaiting': 'Script copied; waiting for VRCX context',
        'status.vrcxBridgeConnected': 'VRCX context received ({seconds}s ago)',
        'status.vrcxBridgeStale': 'VRCX context is stale; run the script again',
        ...
        'label.enableArabicReshaper': 'Reshape Arabic/Hebrew text',
        'hint.enableArabicReshaper': 'Display Arabic and Hebrew with correct forms, direction, and limited manual RTL line breaks in VRChat',
        ...
        'btn.llmTemplateDeepSeekV4Flash': 'DeepSeek v4 Flash (recommended)',
        'btn.llmTemplateOpenRouter': 'OpenRouter',
        ...
        'label.showVrcxBridgeButton': 'Show VRCX context bridge button',
        'hint.showVrcxBridgeButton': 'Copies a script for the VRCX console so the current world and player names can help translation',
        'subsection.vad': 'VAD (Voice Activity Detection)',
        'label.vadEnabled': 'Enable VAD',
        'hint.vadEnabled': 'Master switch for both online API and local recognition. Online API: pauses sending audio during silence to save bandwidth; local: used for automatic segmentation.',
        'label.vadMode': 'VAD Mode (local recognition)',
        'hint.vadMode': 'VAD algorithm used by local recognition; online API gating always uses Silero.',
        'label.vadThreshold': 'VAD Threshold (0.0-1.0)',
        'hint.vadThreshold': 'Higher values are stricter, less likely to treat noise as speech',
        'hint.localVadMovedToAdvanced': 'VAD settings have moved to “Advanced → VAD” and now apply to both online API and local recognition.',
        ...
        'msg.vrcxBridgeScriptCopied': 'VRCX context script copied. Paste it into the VRCX main window console and run it.',
        'msg.vrcxBridgeScriptCopyFailed': 'Failed to copy VRCX context script',
    ...
    'ja': {
        ...
        'btn.stopping': '停止中...',
        'btn.openPanel': 'ミニパネル',
        'btn.copyVrcxBridgeScript': 'VRCX コンテキストスクリプトをコピー',
        'btn.copying': 'コピー中...',
        'btn.clearLanguageInput': '入力をクリア',
        'label.floatingPanelMode': 'フローティングウィンドウ',
        'status.vrcxBridgeIdle': 'VRCX コンテキストはまだ未受信です',
        'status.vrcxBridgeWaiting': 'スクリプトをコピーしました。VRCX からの送信待ちです',
        'status.vrcxBridgeConnected': 'VRCX コンテキスト受信済み（{seconds} 秒前）',
        'status.vrcxBridgeStale': 'VRCX コンテキストが古くなりました。スクリプトを再実行してください',
        ...
        'label.enableArabicReshaper': 'アラビア語/ヘブライ語の表示を整形',
        'hint.enableArabicReshaper': 'VRChat でアラビア語とヘブライ語を正しい字形と方向で表示し、RTL の手動改行数を制限します',
        ...
        'btn.llmTemplateDashscopeQwenFlash': 'Alibaba Qwen3.5-Flash',
        'btn.llmTemplateDashscopeQwenPlus': 'Alibaba Qwen3.5-Plus',
        'btn.llmTemplateDeepSeekV4Flash': 'DeepSeek v4 Flash（推奨）',
        'btn.llmTemplateOpenRouter': 'OpenRouter',
        ...
        'label.showVrcxBridgeButton': 'VRCX コンテキスト橋渡しボタンを表示',
        'hint.showVrcxBridgeButton': 'VRCX コンソール用スクリプトをコピーし、現在のワールド名やプレイヤー名を翻訳補助に使います',
        'subsection.vad': 'VAD（音声区間検出）',
        'label.vadEnabled': 'VAD を有効化',
        'hint.vadEnabled': 'オンライン API とローカル認識の両方を制御する総合スイッチ。オンライン API：無音時に音声送信を停止して通信量を節約。ローカル：自動分割に使用。',
        'label.vadMode': 'VAD モード（ローカル認識）',
        'hint.vadMode': 'ローカル認識で使う VAD アルゴリズム。オンライン API のゲーティングは常に Silero を使用します。',
        'label.vadThreshold': 'VAD しきい値（0.0-1.0）',
        'hint.vadThreshold': '大きいほど厳しくなり、ノイズを音声と誤判定しにくくなります',
        'hint.localVadMovedToAdvanced': 'VAD（音声区間検出）の設定は「詳細設定 → VAD」に移動し、オンライン API とローカル認識の両方に適用されます。',
        ...
        'msg.vrcxBridgeScriptCopied': 'VRCX コンテキストスクリプトをコピーしました。VRCX メイン画面のコンソールに貼り付けて実行してください。',
        'msg.vrcxBridgeScriptCopyFailed': 'VRCX コンテキストスクリプトのコピーに失敗しました',
    ...
    'ko': {
        ...
        'btn.stopping': '중지 중...',
        'btn.openPanel': '미니 패널',
        'btn.copyVrcxBridgeScript': 'VRCX 컨텍스트 스크립트 복사',
        'btn.copying': '복사 중...',
        'btn.clearLanguageInput': '입력 지우기',
        'label.floatingPanelMode': '플로팅 창 모드',
        'status.vrcxBridgeIdle': '아직 VRCX 컨텍스트를 받지 못했습니다',
        'status.vrcxBridgeWaiting': '스크립트를 복사했습니다. VRCX 컨텍스트를 기다리는 중입니다',
        'status.vrcxBridgeConnected': 'VRCX 컨텍스트 수신됨 ({seconds}초 전)',
        'status.vrcxBridgeStale': 'VRCX 컨텍스트가 오래되었습니다. 스크립트를 다시 실행하세요',
        ...
        'label.enableArabicReshaper': '아랍어/히브리어 표시 재정렬',
        'hint.enableArabicReshaper': 'VRChat에서 아랍어와 히브리어가 올바른 글자 모양과 방향으로 표시되게 하고 RTL 수동 줄바꿈 수를 제한합니다',
        ...
        'btn.llmTemplateDashscopeQwenFlash': 'Alibaba Qwen3.5-Flash',
        'btn.llmTemplateDashscopeQwenPlus': 'Alibaba Qwen3.5-Plus',
        'btn.llmTemplateDeepSeekV4Flash': 'DeepSeek v4 Flash (추천)',
        'btn.llmTemplateOpenRouter': 'OpenRouter',
        ...
        'label.showVrcxBridgeButton': 'VRCX 컨텍스트 브리지 버튼 표시',
        'hint.showVrcxBridgeButton': 'VRCX 콘솔용 스크립트를 복사하여 현재 월드와 플레이어 이름을 번역에 활용합니다',
        'subsection.vad': 'VAD(음성 활동 감지)',
        'label.vadEnabled': 'VAD 사용',
        'hint.vadEnabled': '온라인 API와 로컬 인식을 모두 제어하는 메인 스위치. 온라인 API: 무음 시 오디오 전송을 멈춰 대역폭 절약; 로컬: 자동 분할에 사용.',
        'label.vadMode': 'VAD 모드(로컬 인식)',
        'hint.vadMode': '로컬 인식에 사용하는 VAD 알고리즘이며, 온라인 API 게이팅은 항상 Silero를 사용합니다.',
        'label.vadThreshold': 'VAD 임계값 (0.0-1.0)',
        'hint.vadThreshold': '값이 클수록 엄격해져 잡음을 음성으로 오인하기 어려워집니다',
        'hint.localVadMovedToAdvanced': 'VAD 설정이 “고급 설정 → VAD”로 이동했으며 온라인 API와 로컬 인식 모두에 적용됩니다.',
        ...
        'msg.vrcxBridgeScriptCopied': 'VRCX 컨텍스트 스크립트를 복사했습니다. VRCX 메인 창 콘솔에 붙여넣고 실행하세요.',
        'msg.vrcxBridgeScriptCopyFailed': 'VRCX 컨텍스트 스크립트 복사에 실패했습니다',
