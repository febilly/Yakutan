...
const translations = {
    'zh-CN': {
        ...
        'btn.llmTemplateCerebras': 'Cerebras（免费）',
        'hint.llmTemplate': '模板会直接填写下方配置；阿里模板会把当前 DashScope Key 复制到 LLM Key。',
        'hint.llmTemplateKeySource': '{provider} API Key 获取地址：',
        'hint.llmTemplateCerebrasKeyRotation': '支持在下方 LLM API Key 填写多个 key 轮询使用，请用英文逗号 , 分隔。',
        ...
        'option.llmTranslationFormality.customerService': '客服（礼貌过头了）',
        'hint.llmTranslationFormality': '仅影响 LLM 译文的语气和礼貌程度。低更随意，中更自然礼貌，高更郑重正式，客服则接近前台/客服用语。',
    ...
    'en': {
        ...
        'btn.llmTemplateCerebras': 'Cerebras (Free)',
        ...
        'hint.llmTemplateCerebrasKeyRotation': 'You can enter multiple API keys in the LLM API Key field below for rotation. Separate them with commas ,.',
        ...
        'option.llmTranslationFormality.customerService': 'Customer Service (Overly Polite)',
        'hint.llmTranslationFormality': 'Controls the tone and politeness of LLM translations only. Low is casual, medium is naturally polite, high is clearly formal, and customer service uses service-desk level courtesy.',
        'label.llmKey': 'LLM API Key',
    ...
    'ja': {
        ...
        'btn.llmTemplateCerebras': 'Cerebras（無料）',
        ...
        'hint.llmTemplateCerebrasKeyRotation': '下の LLM API Key に複数の key を入力してローテーションできます。半角カンマ , で区切ってください。',
        ...
        'option.llmTranslationFormality.customerService': '接客（丁寧すぎる）',
        'hint.llmTranslationFormality': 'LLM 訳文の語調と丁寧さだけを調整します。低はくだけた会話調、中は自然な丁寧語、高は改まった丁寧語、接客は受付・カスタマーサポート寄りの表現です。',
        'label.llmKey': 'LLM API Key',
    ...
    'ko': {
        ...
        'btn.llmTemplateCerebras': 'Cerebras(무료)',
        ...
        'hint.llmTemplateCerebrasKeyRotation': '아래 LLM API Key 입력란에 여러 key를 넣어 순환 사용할 수 있습니다. 쉼표 , 로 구분하세요.',
        ...
        'option.llmTranslationFormality.customerService': '고객응대 (지나치게 공손함)',
        'hint.llmTranslationFormality': 'LLM 번역의 말투와 공손함만 조절합니다. 낮음은 편한 대화체, 중간은 자연스럽게 공손한 말투, 높음은 더 격식 있는 표현, 고객응대는 안내/상담원 수준의 정중한 표현입니다.',
        'label.llmKey': 'LLM API Key',
